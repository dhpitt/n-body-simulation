...
planet_1 = pointMass(0,1e7,0,-2e6,4.0)
planet_2 = pointMass(-3e7,-5e7,9e5,0,2)
planet_3 = pointMass(2e7,2e7,-4e4,2e5,4)
...
def RK4_step(t,dt,body,scene):
    ...
    y_old = np.array([body.pos_,body.vel_])

    k1 = np.multiply(body.RK4_helper(dt, np.array([0.,0.]), scene), dt) 
    halfk1 = np.multiply(0.5,k1) # only take position for offset
    #k2 = np.multiply(body.RK4_helper(dt, np.multiply(0.5,k1), scene), dt)
    k2 = np.multiply(body.RK4_helper(dt, halfk1[0], scene), dt)
    #k3 = np.multiply(body.RK4_helper(dt, np.multiply(0.5,k2), scene), dt)
    halfk2 = np.multiply(0.5,k2)
    k3 = np.multiply(body.RK4_helper(dt, halfk2[0], scene), dt)
    k4 = np.multiply(body.RK4_helper(dt, k3[0], scene), dt)
    
    y_new = y_old + np.multiply((1/6.),(k1 + np.multiply(k2,2) + np.multiply(k3,2) + k4))
    #y_new = y_old + dt*np.multiply((1/6.),(k1 + np.multiply(k2,2) + np.multiply(k3,2) + k4))
...
#results,tspace = RK4_solve_nBody(0,20,50,planets_3b) # 3 body system output
...
#plt.draw()
plt.draw()
anim.save('2bodytest.mp4', fps=60, extra_args=['-vcodec', 'libx264'])
